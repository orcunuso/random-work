...
function Connect-INGvCenter {
	...
	try {
		if ($Credential) {
			...
			Connect-VIServer -Server $vCenterFQDN -User MYDOMAIN\Svcvcorch -Password sss -WarningAction:SilentlyContinue | Out-Null
		}
	...
	if (Test-Path -Path $Global:InFile) {
		...
		foreach ($Line in $File) {
			...
			switch ($InputParameter) {
				"VCENTER"	{ $Global:vCenter   = $InputValue }
...
if (ReadInputFile) {
	...
	Connect-INGvCenter -vCenter $Global:vCenter
	
	foreach ($Cluster in $Global:Clusters) {
		...
		foreach ($VMHost in $VMHosts) {
			...
			foreach ($vmnic in (Get-VMHostNetworkAdapter -VMHost $VMHost -Physical)) {
	...
	foreach ($VMHost in $VMHosts) {
		Write-INGLog -Message ("    Toplam Path Sayisi (lokal disk dahil):") -NoReturn
		...
		Write-INGLog -Message ("{0}" -f $TotalHostPathCount) -NoDateLog
	}

	Write-INGLog -Message (" ")
	Write-INGLog -Message ("===== TUM TESTLER TAMAMLANDI =====")
