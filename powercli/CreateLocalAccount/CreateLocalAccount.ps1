...
param ( [Parameter(Mandatory=$true)]  [string]$vCenter,
		...
		[Parameter(Mandatory=$true)]  [string]$AccountName,  #svchawk
		[Switch]$Switch )
...
if ($Cluster) { $ESXs = Get-Cluster -Name $Cluster | Get-VMHost | Where {$_.PowerState -eq "PoweredOn"} | Sort Name }
...
foreach ($ESX in $ESXs) {
	...
	if ($ESXVIServer) {
		...
		try {
			$Account    = New-VMHostAccount -UserAccount -GrantShellAccess -Id $AccountName -Description $AccountName -Password ssss -Server $ESXVIServer -Confirm:$false
